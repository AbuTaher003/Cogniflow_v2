-- Create trigger function to validate payment amount based on subscription plan
...
BEGIN
  ...
  IF (plan_slug = 'pro') THEN
    IF (NEW.amount < 400.00) THEN
      RAISE EXCEPTION 'Insufficient payment amount. PRO subscription requires ৳400.';
    END IF;
  ELSIF (plan_slug = 'elite') THEN
    IF (NEW.amount < 1000.00) THEN
      RAISE EXCEPTION 'Insufficient payment amount. ELITE subscription requires ৳1000.';
    END IF;
  END IF;
